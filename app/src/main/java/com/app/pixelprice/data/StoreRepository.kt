...
class StoreRepository: IStoreRepository {
    ...
    private var cachedStoresMap: Map<String, Stores>? = null

    private fun parseStoreJson(): List<Stores> {
        val listType = object : TypeToken<List<Stores>>() {}.type
        return Gson().fromJson(storesJson, listType)
    }

    override suspend fun fetchStores(): List<Stores> {
        return getStoresMap().values.toList()
    }

    override suspend fun getStoresMap(): Map<String, Stores>{
        if (cachedStoresMap == null){
            val storeList = parseStoreJson()
            cachedStoresMap = storeList.associateBy { it.storeID }
        }
        return cachedStoresMap!!
    }

    override suspend fun getStoreById(storeID: String): Stores? {
        val map = getStoresMap()
        return map[storeID]
    }
}